		   ...
		   if ($post["trade_status"] == 'PAY_FINISHED' || $post["trade_status"] == 'TRADE_FINISHED') { // 成功
				...
				else
				{
					...
					if($create_sell_order)
					{
						...
						//新浪代付$user_transfer['poundage']
						$money = $user_transfer['monkey'];
						$fenzhang = $user_transfer['user_id']."^"."UID^SAVING_POT^".sinapay_system."^EMAIL^BASIC^".$user_transfer['poundage']."^债权转让^";
						...
						if($sina_sell[0] != 0)
						{
							// 债券转让代付提交新浪失败：交易订单号为6649201060返回错误重复的请求号,6649201061
